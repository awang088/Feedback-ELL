...
class FeedbackModel(nn.Module):
    def __init__(self, config_path=None, pretrained=False):
        ...
        # self.bilstm = nn.LSTM(self.config.hidden_size, self.config.hidden_size//2, dropout=0, bidirectional=True, batch_first=True)
    ...
    def forward(self, ids, mask, token_type_ids=None):
        ...
        output1 = self.fc(self.dropout1(emb))
        output2 = self.fc(self.dropout2(emb))
        output3 = self.fc(self.dropout3(emb))
        output4 = self.fc(self.dropout4(emb))
        output5 = self.fc(self.dropout5(emb))
