...
MODEL_PATH = '../output/deberta-v3-large-mlm/checkpoint-19000'
...
with timer('deberta-v3-large'):
    ...
    for fold in range(folds):
        ...
        model.load_state_dict(torch.load(
            '../output/deberta-v3-large-mlm/checkpoint-19000/pytorch_model.bin'), strict=False)
        model.to(device)
        ...
        for epoch in range(n_epochs):
            with timer(f'model_fold:{epoch}'):
                ...
                for step, (batch_input_ids, batch_attention_mask, batch_target) in enumerate(train_generator):
                    ...
                    if step % eval_step == 0 or (step+1) == len(train_generator):
                        ...
                        for i, (batch_input_ids, batch_attention_mask, batch_target) in enumerate(valid_generator):
                            ...
                            with torch.no_grad():
                                ...
                                score = calc_loss(batch_target, logits)
                            
                            val_scores.update(score.item(), logits.size(0))
                        ...
                        avg_score = val_scores.avg
                        LOGGER.info(
                            f'Fold: {fold} | Epoch: {epoch} | Step: {step}/{len(train_generator)-1} | Loss: {avg_loss} | Score: {avg_score}'
                        )
...
val_score = MCRMSE(y.values.reshape(-1, 6), oof)
