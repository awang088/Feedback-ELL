...
with timer('deberta-v3-large'):
    ...
    for fold in range(folds):
        ...
        train_datagen = FeedbackDataset(
            x_train['full_text'].values, tokenizer, max_len, y_train.values.reshape(-1,1))
        ...
        valid_datagen = FeedbackDataset(
            x_val['full_text'].values, tokenizer, max_len, y_val.values.reshape(-1,1))
        ...
        for epoch in range(n_epochs):
            with timer(f'model_fold:{epoch}'):
                ...
                if not best_val:
                    ...
                    oof[fold_array == fold] = preds.reshape(-1)
