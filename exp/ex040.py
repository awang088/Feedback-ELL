...
class FeedbackModel(nn.Module):
    def __init__(self, config_path=None, pretrained=False):
        ...
        self.linear1 = nn.Linear(self.config.hidden_size, 1)
        self.linear2 = nn.Linear(self.config.hidden_size, 1)
        self.linear3 = nn.Linear(self.config.hidden_size, 1)
        self.linear4 = nn.Linear(self.config.hidden_size, 1)
        self.linear5 = nn.Linear(self.config.hidden_size, 1)
        self.linear6 = nn.Linear(self.config.hidden_size, 1)
        ...
        self._init_weights(self.linear5)
        self._init_weights(self.linear6)
    ...
    def forward(self, ids, mask, token_type_ids=None):
        emb = self.deberta(ids, attention_mask=mask, token_type_ids=token_type_ids)[
            'last_hidden_state']
        emb = self.pooler(emb, mask)
        output1 = self.linear1(emb)
        output2 = self.linear2(emb)
        output3 = self.linear3(emb)
        output4 = self.linear4(emb)
        output5 = self.linear5(emb)
        output6 = self.linear6(emb)
        output = torch.cat([output1, output2, output3, output4, output5, output6], axis=1)
        return output
