...
PSEUDO_PATH = "../input/fb3/pseudo_df.csv"
...
# Main
# ================================
data_dict = pd.read_csv(TRAIN_PATH).drop(["text_id"], axis=1)
...
with timer('deberta-v3-large'):
    ...
    for fold in range(folds):
        y_pseudo = pseudo_df[y_cols]

        x_train = data_dict.iloc[fold_array != fold].reset_index(drop=True)
        y_train = y.iloc[fold_array != fold].reset_index(drop=True)

        x_train = pd.concat([x_train, pseudo_df]).reset_index(drop=True)
        y_train = pd.concat([y_train, y_pseudo]).reset_index(drop=True)
        ...
        model2 = FeedbackModel(pretrained=True)
        model2.to(device)
            ...
            with timer(f'model_fold:{epoch}'):
                ...
                for step, (batch_input_ids, batch_attention_mask, batch_target) in enumerate(tk0):
                    ...
                    if epoch >= fgm_start_epoch:
                        fgm.attack()
                        with autocast(enabled=True):
                            logits_adv = model(batch_input_ids, batch_attention_mask)
                            loss_adv = criterion(logits_adv, batch_target)
                ...
                if not best_val:
                    ...
                    torch.save(model2.state_dict(), MODEL_PATH_BASE +
                                f"_{fold}_ema.pth")  # Saving the model
                    continue
